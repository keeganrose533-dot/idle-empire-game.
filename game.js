...
function update(){
  ...
  if(xpBar){xpBar.style.width=Math.min((xp/xpNeeded())*100,100)+"%";}
...
function clickMoney(){
  playSound('click');
  let now=Date.now();
  combo = now-lastClick<1500 ? combo+1 : 0;
  lastClick=now;
...
window.onload=()=>{
  ...
  if(save){
    ...
    ascensions = save.ascensions || 0;
    clickPower = save.clickPower || 1;
    autoClick = save.autoClick || 0;
    clickCost = save.clickCost || 50;
    autoCost = save.autoCost || 100;
    tokens = save.tokens || 0;
    tokenUpgrade = save.tokenUpgrade || 0;
    tokenCost = save.tokenCost || 5;
    totalClicks = save.totalClicks || 0;
    totalMoney = save.totalMoney || 0;
    totalAuto = save.totalAuto || 0;
    if(save.achievements){
      achievements.forEach((a,i)=>{ a.unlocked = save.achievements[i] || false; });
    }
  }
